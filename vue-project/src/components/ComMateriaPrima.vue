...
const eliminarMateria = async (idMateria: number) => {
  try {
    ...
    const eliminarMateriaDTO = {
      idMateriaaEliminar: idMateria, // Asegúrate de que este nombre coincida exactamente con el backend
  ...
  // Realizar la solicitud DELETE al backend
    ...
    Swal.fire({
      title: "Error al elminar la materia prima",
...
const openEmergente = (materia) => {
  ...
  materiaSeleccionada.value = materia; // Para la informacion del producto selecionado
  ...
  <transition-group name="fade" tag="div" class="product-container">
    ...
    <div class="card2" v-for="materia in filtrarMateriasPrimas" :key="materia.id">
      ...
      <div class="product-info">
        ...
        <p>Cantidad: <span class="product-quantity">{{ materia.cantidad }} </span></p>
        <p>Stock Minimo: <span class="product-quantity">{{ materia.stock_minimo }}</span></p>
      ...
      <div class="product-buttons">
        ...
        <button @click="openStockModal(materia)" class="add-button">
          Agregar Stock
        </button>
      </div>
