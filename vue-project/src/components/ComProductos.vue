...
const eliminarProducto = async (idProducto: number) => {
  try {
    ...
    const eliminarProductoDTO = {
      ...
      contraseñaJefe: contraseñaJefe,  // Usar la contraseña del jefe
    };

    // Realizar la solicitud DELETE al backend
    const response = await axios.delete("http://localhost:8080/productos/eliminar", {
...
<template>
  ...
  <div class="card2" v-for="producto in productos" :key="producto.id">
    <div class="product-image">
      <img :src="producto.imagen" alt="Imagen del producto" v-if="producto.imagen" />
