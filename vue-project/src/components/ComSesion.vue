...
const validarUsuario = async () => {
  try {
    ...
    const respuesta = await axios.post("http://localhost:8080/usuarios/login", {
      nombre: nombre.value,
      contraseña: contraseña.value,
    });

    // Si la respuesta es exitosa, muestra el mensaje de bienvenida
    successMessage.value = respuesta.data;

    // Aquí se valida el rol y se redirige a la página correspondiente
    const mensaje = respuesta.data; // Mensaje que llega del backend
    if (mensaje.includes("Bienvenido")) {
      // Revisar el rol en el mensaje o respuesta
      if (mensaje.includes("ADMINISTRADOR")) {
        router.push("/Admi");  // Redirige a la ruta para Administradores
      } else if (mensaje.includes("JEFE")) {
        router.push("/Jefe");  // Redirige a la ruta para Jefe
      } else if (mensaje.includes("EMPLEADO")) {
